...
def render_page_image(pdf_path: str, page_num: int, out_dir: Path) -> Path:
...
def extract_page(pdf_path: str, page_num: int, data_dir: Path) -> dict:
    """
    ...
    with pdfplumber.open(pdf_path) as pdf:
        raw = pdf.pages[page_num].extract_text() or ""
    ...
    img_path = render_page_image(pdf_path, page_num, data_dir)
    try:
...
def extract_text(pdf_path: str) -> list[dict]:
    ...
    for i in range(num_pages):
        log.info(f"\n--- Page {i + 1} / {num_pages} ---")
        try:
            result = extract_page(pdf_path, i, data_dir)
